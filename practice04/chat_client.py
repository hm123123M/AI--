...
def send_request(protocol, host, port, path, api_key, messages, temperature=0.7, max_tokens=4096, stream=True):
    ...
    for attempt in range(max_retries):
        try:
            if protocol == 'https':
                conn = http.client.HTTPSConnection(host, port, timeout=base_timeout)
            else:
                conn = http.client.HTTPConnection(host, port, timeout=base_timeout)
...
def execute_tool_call(tool_call):
    ...
    try:
        ...
        if tool_name == "list_files":
            ...
            return tool_func(parameters.get("query", ""))
...
def compress_history(protocol, host, port, path, api_key, history, temperature=0.7, max_tokens=4096):
    ...
    split_index = int(len(dialog_messages) * 0.7)
    
    if split_index <= 0:
        return history
    
    messages_to_compress = dialog_messages[:split_index]
    messages_to_keep = dialog_messages[split_index:]
    
    summary_prompt = """请对以下对话历史进行简明扼要的总结，保留关键信息和对话要点：
...
请提供总结："""
    ...
    summary_request = [
        ...
        {"role": "user", "content": summary_prompt.format(dialog=dialog_text)}
    ]
    ...
    for attempt in range(max_retries + 1):
        ...
        response, conn, status = send_request_with_timeout(
            ...
            summary_request, temperature, max_tokens, 
    ...
    original_length = calculate_context_length(history)
    new_length = calculate_context_length(new_history)
    compression_ratio = (1 - new_length / original_length) * 100
    
    print(f"[压缩完成] 原长度: {original_length} 字符, 新长度: {new_length} 字符, 压缩率: {compression_ratio:.1f}%")
    
    return new_history

# ================ 5W关键信息提取相关函数 ================

def extract_5w_info(protocol, host, port, path, api_key, history, temperature=0.7, max_tokens=4096):
    """提取聊天历史中的5W关键信息（Who、What、When、Where、Why）"""
    print("\n[每5轮聊天，提取5W关键信息...]")
    ...
    dialog_text = ""
    for msg in dialog_messages[-EXTRACT_INTERVAL*2:]:
        role = "用户" if msg['role'] == 'user' else "AI"
        dialog_text += f"{role}: {msg['content']}\n"
...
请输出JSON格式：[{{"Who":"谁","What":"做了什么","When":"时间","Where":"地点","Why":"原因"}}]"""
    ...
    if 'choices' in response_data and len(response_data['choices']) > 0:
        choice = response_data['choices'][0]
        if 'message' in choice and 'content' in choice['message']:
            extract_result = choice['message']['content']
    
    if not extract_result.strip():
        print("5W信息提取结果为空")
        return
    ...
    try:
        while True:
            ...
            if should_compress_history(history):
                history = compress_history(protocol, host, port, path, API_KEY, history, TEMPERATURE, MAX_TOKENS)
            ...
            print(f"\n\n[耗时: {elapsed_time:.2f}秒] [历史轮数: {total_rounds}] [上下文长度: {calculate_context_length(history)}]\n")
